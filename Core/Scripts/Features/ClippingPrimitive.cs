    ...
    public abstract class ClippingPrimitive : MonoBehaviour, IMaterialInstanceOwner
    {
        ...
        public void AddRenderer(Renderer _renderer)
        {
            if (_renderer != null)
            {
                ...
                var matInstance = EnsureComponent<MaterialInstance>(_renderer.gameObject);
                ToggleClippingFeature(matInstance.AcquireMaterials(this), gameObject.activeInHierarchy);
            }
        }
        ...
        protected void ToggleClippingFeature(bool keywordOn)
        {
            if (renderers != null)
            {
                for (var i = 0; i < renderers.Count; ++i)
                {
                    ...
                    if (_renderer != null)
                    {
                        var materialInstance = EnsureComponent<MaterialInstance>(_renderer.gameObject);
        ...
        {
            var foundComponent = go.GetComponent<T>();
            return foundComponent == null ? go.AddComponent<T>() : foundComponent;
        }
    }
}
